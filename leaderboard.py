...
async def create_leaderboard_embed(bot, guild: discord.Guild, page: int = 1, per_page: int = 10) -> discord.Embed:
    ...
    for idx, (user_id, pts) in enumerate(sorted_points[start:end], start=start + 1):
        ...
        if member:
            ...
            # fetch user from Discord API if not cached
...
class LeaderboardView(discord.ui.View):
    ...
    async def prev_page(self, button: discord.ui.Button, interaction: discord.Interaction):
        if self.current_page <= 1:
            ...
            return
        self.current_page -= 1
        embed, _ = await create_leaderboard_embed(self.bot, self.guild, self.current_page, self.per_page)
        self._sync_buttons()
...
class PointsModule(commands.Cog):
    ...
    async def leaderboard(
        ...
        embed, total_pages = await create_leaderboard_embed(self.bot, guild, page, 10)
        view = LeaderboardView(self.bot, guild, page, total_pages, 10)
