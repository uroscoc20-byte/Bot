...
class Admin(commands.Cog):
    """Admin commands for managing points and bot configuration"""
    ...
    async def points_add(
        ...
        new_points = await self.bot.db.add_points(user.id, amount)
    ...
    async def points_remove(
        ...
        current_points = await self.bot.db.get_points(user.id)
        new_points = await self.bot.db.remove_points(user.id, amount)
    ...
    @app_commands.command(name="points_set", description="Set user's points to exact value (Admin only)")
    @app_commands.describe(
    ...
    async def points_set(
        ...
        await self.bot.db.set_points(user.id, amount)
    ...
    async def points_reset(self, interaction: discord.Interaction):
        ...
        view = ConfirmResetView(self.bot, interaction.user)
    ...
    @app_commands.command(name="points_remove_user", description="Remove a user from the leaderboard (Admin only)")
    @app_commands.describe(user="User to remove from leaderboard")
    async def points_remove_user(self, interaction: discord.Interaction, user: discord.Member):
        ...
        deleted = await self.bot.db.delete_user_points(user.id)
    ...
    @app_commands.command(name="ticket_kick", description="Remove a helper from the current ticket")
    @app_commands.describe(user="Helper to remove from this ticket")
    async def ticket_kick(self, interaction: discord.Interaction, user: discord.Member):
        ...
        # Update ticket embed
        from cogs.tickets import create_ticket_embed
