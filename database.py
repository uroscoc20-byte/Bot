...
class Database:
    ...
    async def save_config(self, key: str, value):
        """Save config value (dict, string, etc.)"""
    ...
    async def load_config(self, key: str):
        """Load config value"""
        if self.backend == "firestore":
            try:
                def _op():
    ...
    async def add_points(self, user_id: int, amount: int) -> int:
        ...
        await self.set_points(user_id, new_total)
        return new_total

    async def remove_points(self, user_id: int, amount: int) -> int:
        """Remove points from user (won't go below 0)"""
        current = await self.get_points(user_id)
    ...
    async def get_leaderboard(self, limit: int = 100):
        ...
        if self.backend == "firestore":
            try:
                def _op():
                    docs = self.fs.collection("user_points").order_by("points", direction=firestore.Query.DESCENDING).limit(limit).stream()
                    return [d.to_dict() for d in docs]
                return await self._fs_run(_op)
            except Exception as e:
                await self._fallback_to_sqlite(str(e))
        
        async with self.db.execute("SELECT user_id, points FROM user_points ORDER BY points DESC LIMIT ?", (limit,)) as cursor:
    ...
    async def delete_user_points(self, user_id: int) -> bool:
    ...
    async def save_ticket(self, ticket_data: dict):
        """Save or update active ticket"""
        if self.backend == "firestore":
            try:
                def _op():
                    channel_id = ticket_data["channel_id"]
                    self.fs.collection("active_tickets").document(str(channel_id)).set(ticket_data)
                return await self._fs_run(_op)
            except Exception as e:
                await self._fallback_to_sqlite(str(e))
        ...
        """, (
            ...
            1 if ticket_data.get("proof_submitted", False) else 0,
            ticket_data.get("proof"),
            ticket_data.get("embed_message_id"),
            ticket_data.get("in_game_name"),
            ticket_data.get("concerns"),
            selected_bosses_json,
            ticket_data.get("selected_server"),
            1 if ticket_data.get("is_closed", False) else 0
        ))
        await self.db.commit()
    ...
    async def save_ticket_history(self, history_data: dict):
