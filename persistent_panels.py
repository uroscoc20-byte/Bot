...
@commands.slash_command(
    name="persistent_leaderboard",
    description="Create a persistent leaderboard that auto-refreshes"
)
async def persistent_leaderboard(
    ...
    # Create the embed
    embed = await create_leaderboard_embed(page=page, per_page=per_page)
    # Force the title to Season 8
    embed.title = "🏆 HELPER'S LEADERBOARD SEASON 8 🏆"
    ...
    await ctx.followup.send(
        "✅ **Persistent leaderboard created!** It will auto-refresh every 10 minutes.",
        ephemeral=True
    )
